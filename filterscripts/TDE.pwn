...
new
    ...
    line[800], bool:MouseCursor, bool:PixelLock, SpeedSTR[256]
;
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	if(!strcmp(cmdtext, "/tde", true, 4))
	{
	    ...
	    if(ProjectEditor == -1)
	    {
	        ...
	        DestroyMenuTextDraws();
  			CreateMenuTextDraws();
	        SendClientMessage(playerid, -1, "{e2b960}TDEditor 1.16 {FFFFFF}Click on the image to start...");
...
stock DuplicateTextDraw(Index, ToIndex)
{
	...
	if(TDE_TextDrawGetAlignment(ProjectTD[Index][ETextDrawID]) == 2) TDE_TextDrawTextSize(ProjectTD[ToIndex][ETextDrawID], ProjectTD[ToIndex][ETextDrawTextY], ProjectTD[ToIndex][ETextDrawTextX]);
...
stock ExportProject()
{
	...
	format(line, sizeof(line), "\r\nTDEditor V1.16 by adri1"); fwrite(ExportIO, line);
	fclose(ExportIO);
