...
| 2020.05.19 | 
  ...
  |      | |___|_ |___| |     |___| |    |____ |____  
                  | __________________________________
                                   __        
                    |\/|  _    _  |__  _| * _|_  _  |__
                    |  | |_|_ |_| |__ |_| |  |  |_| |    | v1.0 | All in one sophisticated Map Editor! 

_______ Credits _______
[MapForce]xCrafter - for thr opportunity
GameOvr - Scripter
...
/* MapEditor Defines*/
#define 	   MAP_SAVE_PATH				            "/MapEditor/Saves/%s.ini"
#define 	   MAP_EXPORT_PATH					        "/MapEditor/Exports/%s.txt"
#define 	   MAP_EDITOR_CONF_PATH				        "/MapEditor/MapEditor_Data.ini"
...
enum ActorInfo
{
    STREAMER_TAG_ACTOR A_ID,
	A_SKIN,
	STREAMER_TAG_3D_TEXT_LABEL A_LID,
	Float:A_PosX,
	Float:A_PosY,
	Float:A_PosZ,
	Float:A_Ang,
...
GetXYInFrontOfPlayer(playerid, &Float:x, &Float:y, Float:distance) // credits -> Y_Less 
{
    new Float:a;
...
CancelFlyMode(playerid)
{
    ...
    SetTimerEx("SetPos", 500, 0, "i", playerid);
    CancelDynamicEdit(playerid);
    return 1;
}
...
GetServerMapID(mname[])
{
	...
	print("|MapForce| ID for the given map name not found! (Function : GetServerMapID(mname[]))");
	return 1;
}
...
GetPlayerMapID(playerid, mname[])
{
	...
	print("|MapForce| ID for the given map name not found! (Function : GetPlayerMapID(playerid, mname[]))");
	return 1;
}
...
public OnFilterScriptInit()
{
    ...
    print("\n\n\n|=======================================\n");
    print("| 'MapEditor' successfully Loaded!\n");
    print("|---------------------------------------------\n");
...
public SaveMappings()
{
	...
	print("|MapForce| Maps have been automatically saved for safety");
	return 1;
}
...
public OnFilterScriptExit()
{
	...
	print("\n--------------------------------------");
    print("MapEditor by GameOvr Unoaded!");
    print("--------------------------------------\n");
	return 1;
}
...
{
    SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce| "#COL_GREY"Use "#COL_YELLOW"/editorcmds "#COL_GREY"to view "#COL_GREEN"MapEditor "#COL_GREY"commands");
    return 1;
}
...
public OnPlayerEditDynamicObject(playerid, STREAMER_TAG_OBJECT objectid, response, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz)
{
    if(IsValidDynamicObject(objectid))
    {
        if(response == EDIT_RESPONSE_FINAL)
        {
            ...
            new str[128]; 
            format(str, sizeof(str), ""#COL_BLUE"|MapForce| "#COL_YELLOW"New locaion for object id %i in map '%s' has been saved", PLAYER_INFO[playerid][cur_obj], PLAYER_mName[playerid][PLAYER_INFO[playerid][cur_player_mid]]);
        ...
        else if(response == EDIT_RESPONSE_CANCEL)
        {
            ...
            new str[128];
            format(str, sizeof(str), ""#COL_BLUE"|MapForce| "#COL_YELLOW"New locaion for object %i in map '%s' hasn't been saved!", PLAYER_INFO[playerid][cur_obj], PLAYER_mName[playerid][PLAYER_INFO[playerid][cur_player_mid]]);
            SendClientMessage(playerid, -1, str);
        }
    }
    return 1;
...
public OnDynamicObjectMoved(STREAMER_TAG_OBJECT objectid)
{
    ...
    if(id != -1)
    {
        SetDynamicObjectPos(objectid, OBJ_INFO[mid][id][O_PosX], 
        OBJ_INFO[mid][id][O_PosY],  
        OBJ_INFO[mid][id][O_PosZ]); 

        MoveDynamicObject(objectid,
        OBJ_INFO[mid][id][movex], 
        OBJ_INFO[mid][id][movey], 
        OBJ_INFO[mid][id][movez],  
        OBJ_INFO[mid][id][ospeed], 
        OBJ_INFO[mid][id][O_RotX], 
        OBJ_INFO[mid][id][O_RotY],  
        OBJ_INFO[mid][id][O_RotZ]); 
    }
    return 1;
}

/* MapEditor Commands */ 
CMD:editorcmds(playerid, params[])
{       
    new str[2500]; 
...
CMD:cm(playerid, params[])
{
	...
	if(count >= MAX_MAPS_THAT_CAN_BE_CREATED_BY_A_USER) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You have reached the maximum number of maps that can be created by a player ( limit: "#MAX_MAPS_THAT_CAN_BE_CREATED_BY_A_USER" )");
	if(sscanf(params, "s[20]s[20]", mname, mpass)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /cm <name> <password>");
	if(strlen(mname) < 4 || strlen(mname) > 20) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Map name should have 4 - 20 characters");
	if(strlen(mpass) < 4 || strlen(mpass) > 20) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Map password should have 4 - 20 characters");
	if(!IsValidMapName(mname)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Map name should only contain lower case letters or numbers");
	format(str, sizeof(str), MAP_SAVE_PATH, mname);
	if(dini_Exists(str)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" A map with this name already exists");
	SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_YELLOW" Your map has been successfully created");
...
{
    new mname[20], mpass[20];
    if(PLAYER_INFO[playerid][mLoaded] == MAX_LOADABLE_MAPS_AT_ONCE) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You have reached the limit of the maximum  no of maps which can be open at once (Limit: "#MAX_LOADABLE_MAPS_AT_ONCE")");
    if(EDITOR_INFO[M_Loaded] == MAX_LOADABLE_MAPS_IN_SERVER) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Server has reached the limit of the maximum  no of maps which can be loaded at once (Limit: "#MAX_LOADABLE_MAPS_IN_SERVER")");
    if(sscanf(params, "s[20]s[20]", mname, mpass)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /lm <name> <password>");
    if(!IsValidMapName(mname)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Map name should only contain lower case letters or numbers");
    for(new i = 0; i < MAX_LOADABLE_MAPS_AT_ONCE; i ++)
    {
        if(isequal(PLAYER_mName[playerid][i], mname)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You have already loaded this map");
    }
    for(new i = 0; i < MAX_LOADABLE_MAPS_IN_SERVER; i++)
    {
        if(isequal(MAP_INFO[i][M_Name], mname)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Someone has loded this map for editing. You can't load the map until he/she finished editing.");
    }
    new str[128];
    format(str, sizeof(str), MAP_SAVE_PATH, mname);
    if(!dini_Exists(str)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" A map with this name does not exist");
    if(!isequal(mpass, dini_Get(str, "Password"))) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Wrong password!");
    SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_GREY" Your map has been successfully loaded");
    for(new i = 0; i < MAX_LOADABLE_MAPS_AT_ONCE; i ++)
    {
    ...
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" %i objects, %i actors and %i vehicles loaded! ( Map name: %s | Map ID: %i )",  MAP_INFO[id_][total_obj], MAP_INFO[id_][total_actors], MAP_INFO[id_][total_vehicles], mname, GetPlayerMapID(playerid, mname));
    SendClientMessage(playerid, -1, str);
...
{
    new mname[20], mpass[20];
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps");
    if(sscanf(params, "s[20]s[20]", mname, mpass)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /ulm <name> <password>");
    if(strlen(mname) < 4 || strlen(mname) > 20) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Map name should have 4 - 20 characters");
    if(!IsValidMapName(mname)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Map name should only contain lower case letters or numbers");
    new str[128];
    format(str, sizeof(str), MAP_SAVE_PATH, mname);
    ...
    if(ret == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" This map isn't loaded");
    if(!isequal(mpass, dini_Get(str, "Password"))) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Wrong password!");
    new id_ = GetServerMapID(mname);    
    SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_GREY" Your map has been successfully unloaded");
    ...
    format(str, sizeof(str), ""#COL_BLUE"|MapForce| "#COL_YELLOW"%i objects, %i actors and %i vehicles unloaded! ( Map name: %s | Map ID: %i )", MAP_INFO[id_][total_obj], MAP_INFO[id_][total_actors], MAP_INFO[id_][total_vehicles], mname, id__);
    SendClientMessage(playerid, -1, str);
...
CMD:ulm(playerid, params[])
{
	...
	if(!isequal(mpass, dini_Get(str, "Password"))) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Wrong password!");
    new id_ = GetServerMapID(mname);
    if(MAP_INFO[id_][forview]) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" This map is loaded only for view! use /ulmv instead");
    ...
    for(new i = 0; i < MAX_PLAYERS; i++)
    {
        for(new j = 0; j < MAX_LOADABLE_MAPS_AT_ONCE; j++)
        {
            if(isequal(PLAYER_mName[i][j], mname))
            {
                loader = i;
                format(PLAYER_mName[loader][j], 20, "%s", "-1");
                break;
            }
        }
        if(loader != -1) break;
	...
	if(loader != playerid)
    {
        format(str, sizeof(str), ""#COL_BLUE"|MapForce| "#COL_YELLOW"%i objects, %i actors and %i vehicles unloaded! ( Map name: %s | Loader: %s )", MAP_INFO[id_][total_obj], MAP_INFO[id_][total_actors], MAP_INFO[id_][total_vehicles], mname, PlayerName(loader));
	    SendClientMessage(playerid, -1, str);
    
    else format(str, sizeof(str), ""#COL_BLUE"|MapForce| "#COL_YELLOW"%i objects, %i actors and %i vehicles unloaded! ( Map name: %s | Map ID: %i )", MAP_INFO[id_][total_obj], MAP_INFO[id_][total_actors], MAP_INFO[id_][total_vehicles], mname);
...
{		
	if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
	new str[1024];
	...
	ShowPlayerDialog(playerid, DIALOG_EDIT_MAP, DIALOG_STYLE_TABLIST_HEADERS, "MapForce - Choose a Map for editing", str, "Edit", "Close");
	return 1;
}
...
{
    new mname[20], mpass[20];
    if(sscanf(params, "s[20]s[20]", mname, mpass)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /export <map name> <map password>");
    new str[128];
    format(str, sizeof(str), MAP_SAVE_PATH, mname);
    if(!dini_Exists(str)) return SendClientMessage(playerid, -1,  ""#COL_BLUE"|MapForce|"#COL_RED" A map with this name does not exist");
    if(!isequal(mpass, dini_Get(str, "Password"))) return SendClientMessage(playerid, -1,  ""#COL_BLUE"|MapForce|"#COL_RED" Wrong password!");
    ...
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_GREY" Map '%s' exported to '/scriptfiles%s'", mname, str_);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:mpass(playerid, params[])
{
	...
	if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
	if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
	if(sscanf(params, "s[20]s[20]", opass, npass)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /mpass <old password> <new password>");
	if(strlen(npass) > 20 || strlen(npass) < 4) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Map password should contain 4 - 20 characters");
	...
	if(!isequal(opass, ropass)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Wrong password!");
	if(strlen(npass) > 20 || strlen(npass) < 4) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Map password shoul contain 4 - 20 characters");
	dini_Set(str, "Password", npass);
	format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_GREY" You changed the password of map '%s' ( new password: %s )", PLAYER_mName[playerid][PLAYER_INFO[playerid][cur_player_mid]], npass);
	SendClientMessage(playerid, -1, str);
	return 1; 
}
...
CMD:mr(playerid, params[])
{
	...
	if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
	if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
	if(sscanf(params, "s[20]", nname)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /mrename <new name>");
	if(strlen(nname) > 20 || strlen(nname) < 4) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Map name should contain 4 - 20 characters");
	if(!IsValidMapName(nname)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Map name can contain only lowercase letters with numbers");
	...
	format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_GREY" You changed the name of map '%s' ( new name: %s )", PLAYER_mName[playerid][PLAYER_INFO[playerid][cur_player_mid]], nname);
	SendClientMessage(playerid, -1, str);
...
{
    new name[20], pass[20];
    if(sscanf(params, "s[20]s[20]", name, pass)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /md <map name> <map pass>");
    if(isequal(name, "-1")) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" A map with this name does not exist!");

    for(new i = 0; i < MAX_LOADABLE_MAPS_IN_SERVER; i++)
    {
        if(isequal(name, MAP_INFO[i][M_Name])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Someone has loaded this map, You can't delete it until he/she unloads the map!");
    }
    ...
    for(new i = 0; i < MAX_CREATABLE_MAPS_IN_SERVER; i++)
    {
        ...
        if(dini_Isset(MAP_EDITOR_CONF_PATH, str))
        {
            ...
            {
                format(path, sizeof(path), MAP_SAVE_PATH, dini_Get(MAP_EDITOR_CONF_PATH, str));
                if(!isequal(dini_Get(path, "Password"), pass)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Wrong password!");
                dini_Remove(path);
    ...
    if(!removed) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" A map with this name does not exist!");
    SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_GREY" Map deleted successfully!");
    return 1;
}
...
CMD:maps(playerid, params[])
{
    ...
    if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You don't have permissons to use this command");
    for(new i = 0; i < MAX_MAPS_THAT_CAN_BE_CREATED_BY_A_USER; i++)
    {
        format(mDialog[playerid][i], 20, "%s", "-1");
    }
    if(sscanf(params, "s[24]", name)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /maps <name>");
    new str[128];
    ...
    if(val == -1)
    {
        format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_RED" Player '%s' haven't created any maps", name);
        return SendClientMessage(playerid, -1, str);
    }
    SaveMapData();
    format(str, sizeof(str), "MapForce - Maps created by '%s'", name);
...
{
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any map for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing"); 
    if(PLAYER_INFO[playerid][labeled] == 0)
    {
        ...
        SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_GREY" Labels for current map has been added successfully!");
    }
    else
    {
        PLAYER_INFO[playerid][labeled] = 0;
        ...
        SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_GREY" Labels for current map has been removed successfully!");
    }
...
CMD:fontlist(playerid, params[])
{
    ...
    ShowPlayerDialog(playerid, DIALOG_CLOSE, DIALOG_STYLE_MSGBOX, "MapForce - Fonts", str1, "Close", "");
    return 1;
}
...
{
    ShowPlayerDialog(playerid, DIALOG_MODEL_SEARCH, DIALOG_STYLE_INPUT, "MapForce - Search Object Models", "Input the whole name or part of the name\nof any object", "Search", "Close");
    return 1;
}
...
CMD:odc(playerid, params[])
{
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(!IsValidDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    
    DestroyDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_ID]);

    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You deleted the object ID %i ( model ID: %i )", PLAYER_INFO[playerid][cur_obj], OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_MODEL_ID]);
    SendClientMessage(playerid, -1, str);
...
CMD:od(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "i", id)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /od <object ID>");
    if(id < 0 || id > MAX_OBJ) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid Object ID");
    if(!IsValidDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][O_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid object ID");
    if(id == PLAYER_INFO[playerid][cur_obj]) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You are currently editing this object ( use /od )");

    DestroyDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][O_ID]);

    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You deleted the object ID %i ( model ID: %i )", id, OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][O_MODEL_ID]);
    SendClientMessage(playerid, -1, str);
...
CMD:ox(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(!IsValidDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(sscanf(params, "f", X)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /ox <X cordinate>");
    ...
    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You have changed the X cordinate of Object ID %i by %f", PLAYER_INFO[playerid][cur_obj], X);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:oy(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(!IsValidDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(sscanf(params, "f", Y)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /oy <Y cordinate>");
    ...
    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You have changed the Y cordinate of Object ID %i by %f", PLAYER_INFO[playerid][cur_obj], Y);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:oz(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(!IsValidDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(sscanf(params, "f", Z)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /oz <Z cordinate>");
    ...
    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You have changed the Z cordinate of Object ID %i by %f", PLAYER_INFO[playerid][cur_obj], Z);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:rx(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(!IsValidDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(sscanf(params, "f", rx)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /rx <X rotation>");
    ...
    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You have changed the X rotation of Object ID %i by %f", PLAYER_INFO[playerid][cur_obj], rx);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:ry(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(!IsValidDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(sscanf(params, "f", ry)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /ry <Y rotation>");
    ...
    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You have changed the Y rotation of Object ID %i by %f", PLAYER_INFO[playerid][cur_obj], ry);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:rz(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(!IsValidDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(sscanf(params, "f", rz)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /rz <Z rotation>");
    ...
    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You have changed the Z rotation of Object ID %i by %f", PLAYER_INFO[playerid][cur_obj], rz);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:aox(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "f", X)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /aox <X cordinate>");
    ...
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You have changed the X cordinate of all objects by %f", X);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:aoy(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "f", Y)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /aoy <Y cordinate>");
    ...
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You have changed the Y cordinate of all objects by %f", Y);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:aoz(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "f", Z)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /aoz <Z cordinate>");
    ...
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You have changed the Z cordinate of all objects by %f", Z);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:arx(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "f", rx)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /arx <X rotation>");
    ...
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You have set the X rotation of all objects by %f", rx);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:ary(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "f", ry)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /ary <Y rotation>");
    ...
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You have set the Y rotation of all objects by %f", ry);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:arz(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "f", rz)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /arz <Z rotation>");
    ...
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You have set the Z rotation of all objects by %f", rz);
    SendClientMessage(playerid, -1, str);
    return 1;
}                                             
...
CMD:oc(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any map for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(MAP_INFO[PLAYER_INFO[playerid][cur_player_mid]][total_obj] == MAX_OBJ) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You have reached the maximum amout of objects that can be created in one map (Limit: "#MAX_OBJ")");
    if(sscanf(params, "i", model)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /oc <model ID>");
    if(!IsIDEValid(model)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid model ID");
    ...
    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Object successfully created! ( Map ID: %s(%i) | Object ID: %i | Model ID: %i )", MAP_INFO[PLAYER_INFO[playerid][cur_svr_mid]][M_Name], PLAYER_INFO[playerid][cur_player_mid], oid, model);
    SendClientMessage(playerid, -1, str);
...
CMD:ocopy(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any map for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "i", id)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /ocopy <object ID>");    
    if(id < 0 || id >= MAX_OBJ) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid object ID");   
    if(!IsValidDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][O_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid object ID");   
...
CMD:opaste(playerid, params[])
{
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any map for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(!IsValidDynamicObject(OBJ_INFO[COPY_BUF[playerid][mapid]][COPY_BUF[playerid][objid]][O_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Data object copied has been destroyed! Unable to sync!!!");   
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    new str[128];
    for(new k = 0; k < MAX_INDEX; k++)
    {
        ...
        if(Mat_TUSED[COPY_BUF[playerid][mapid]][COPY_BUF[playerid][objid]][k])
        {
            ...
            Mat_TUSED[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][k] = 1;
...
CMD:omove(playerid, params[])
{
    ...
    if(sscanf(params, "ifffi", id, ox, oy, oz, sec)){
        SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /omove <object ID> <x> <y> <z> <seconds>");
        return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Time should be given in 'seconds'");
    }
    if(id < 0 || id >= MAX_OBJ) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid object ID");
    if(!IsValidDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][O_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid object ID");
    if(ox == 0 && oy == 0 && oz == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Object cannot move with '0' distance");
    if(sec == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Object cannot move in '0' seconds");
    ...
    OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][ospeed] = (GetDistance(
    ...
    MoveDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][O_ID], 
        ...
        OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][O_RotX], 
        OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][O_RotY],  
        OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][O_RotZ]); 
    return 1;
}
...
CMD:ostopmove(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any map for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "i", id)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /ostopmove <object ID>");
    if(!OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][move])  return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" This object is not moving!, use /omove");
    if(id < 0 || id >= MAX_OBJ) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid object ID");
    if(!IsValidDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][O_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid object ID");
...
CMD:oclone(playerid, params[])
{
    ...
    for(new i = 0; i < MAX_OBJECTS; i++)
    {
        ...
        {
            oid = i;
            OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][oid][O_ID] = CreateDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_MODEL_ID], 0, 0, 0, 0, 0, 0);
            break;  
        }
    }
    ...
    for(new k = 0; k < MAX_INDEX; k++)
    {
        ...
        if(Mat_TUSED[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][k])
        {
            ...
            Mat_TUSED[PLAYER_INFO[playerid][cur_svr_mid]][oid][k] = 1; 
    ...
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Object successfully cloned! ( Map ID: %s(%i) | Object ID: %i | Model ID: %i )", MAP_INFO[PLAYER_INFO[playerid][cur_svr_mid]][M_Name], PLAYER_INFO[playerid][cur_player_mid], oid, OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_MODEL_ID]);
    SendClientMessage(playerid, -1, str);
...
CMD:omodel(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(sscanf(params, "i", model)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /omodel <new model ID>");
    if(!IsIDEValid(model)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid model ID");
    if(model == OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_MODEL_ID]) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" This the same model ID that object holds now!"); 
...
CMD:oe(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any map for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing"); 
    if(PLAYER_INFO[playerid][editing] == 1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You are editing an object, finish it before editing another");
    if(sscanf(params, "i", id)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /oe <object ID>");
    if(id > MAX_OBJ  || id < 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid object ID");
    if(!IsValidDynamicObject(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][O_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid object ID");
    if(PLAYER_INFO[playerid][cur_obj] == id) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You have already selected this object for editing");
    
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You are now editing object ID %i ", id);
    SendClientMessage(playerid, -1, str);
...
CMD:op(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing"); 
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing"); 
    if(PLAYER_INFO[playerid][editing] == 1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You are already editing an object, finish it to edit another"); 
    ...
    SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_GREY" You can move the camera while editing by pressing and holding the spacebar (or W in vehicle) and moving your mouse.");
    SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_GREY" You have to click on the save icon to save your updates & press ESC to cancel");
    return 1;
}
...
CMD:fontsearch(playerid, params[])
{
    if(!fsearch[playerid])
    {
        ...
        SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_GREY" Use space + Y and space + N to switch fonts");
    }
    else 
    {
        fsearch[playerid] = 0;
...
CMD:mattexture(playerid, params[])
{
    ...
    if(sscanf(params, "ii", index, mid)){
        SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /mattexture <index> <material ID>");
        return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_GREY" Use /tbrowse to view material IDs");
    }
    if(index < 0 || index >= MAX_INDEX) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid index (Index should be in between 0 - 15)");
    if(mid > MAX_TEXTURES || mid < 0){
        SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid material ID");
        return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_GREY" Use /tbrowse to view material IDs");
    }
    ...
    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Material of index %i in object ID %i has been changed to Material ID %i", index, PLAYER_INFO[playerid][cur_obj], mid);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:mattextureall(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any map for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing"); 
    if(MAP_INFO[PLAYER_INFO[playerid][cur_svr_mid]][total_obj] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You don't have any object in this map!"); 
    if(sscanf(params, "ii", index, mid)){
        SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /mattextureall <index> <material ID>");
        return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_GREY" Use /tbrowse to view material IDs");
    }
    if(index < 0 || index >= MAX_INDEX) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid index (Index should be in between 0 - 15)");
    if(mid > MAX_TEXTURES || mid < 0){
    ...
    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Material of index %i in all objects has been changed to Material ID %i", index, mid);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:matreset(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing"); 
    if(MAP_INFO[PLAYER_INFO[playerid][cur_svr_mid]][total_obj] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You don't have any object in this map!"); 
    if(sscanf(params, "i", index)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /matreset <index>");
    if(index < 0 || index >= MAX_INDEX) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid index (Index should be in between 0 - 15)");
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing"); 
    if(!Mat_USED[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][index]) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't used any material in that object!");
    ...
    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Material of index %i in object ID %i has been reset", index, PLAYER_INFO[playerid][cur_obj]);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:matcolor(playerid, params[])
{
    ...
    if(sscanf(params, "ih", index, hex)){
        SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /matcolor <index> <color>");
        return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_GREY" Type the hex of the color, (Ex - Hex of white is 0xFFFFFFFF |  0xAARRGGBB)");
    }
    if(index < 0 || index >= MAX_INDEX) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid index (Index should be in between 0 - 15)");
    ...
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Material color of index %i in object ID %i has been changed to %x", index, PLAYER_INFO[playerid][cur_obj], hex);
...
CMD:mattext(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing"); 
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing"); 
    if(sscanf(params, "is["#MAX_MATTEXT_SIZE"]", index, str)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /mattext <index> <text>");
    if(strlen(str) > MAX_MATTEXT_SIZE || strlen(str) < 1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid character length");
    Mat_TUSED[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][index] = 1;
    Mat_USED[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][index] = 0;
    SetDynamicObjectMaterialText(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_ID], index, str, 
        Mat_TMSIZE[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][index], 
    ...
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Material text of index %i in object ID %i has been changed to \"%s\"", index, PLAYER_INFO[playerid][cur_obj], str);
    SendClientMessage(playerid, -1, str1);
    return 1;
}
...
CMD:matalign(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing"); 
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing"); 
    if(sscanf(params, "ii", index, align)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /matalign <index> <align>");
    if(align < 1 || align > 3) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid align id (Left : 1 | Center : 2 | Right : 3)");
    if(index < 0 || index >= MAX_INDEX) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid index");
    if(!IsMatTextUsedInIndex(PLAYER_INFO[playerid][cur_svr_mid], PLAYER_INFO[playerid][cur_obj], index)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't marked any text to bold!");
    new str1[128];
    switch(align)
    {
        case 1:{
            Mat_TALIGN[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][index] = 0;
            format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Material alignment of index %i in object ID %i has been set to LEFT", index, PLAYER_INFO[playerid][cur_obj]);
    
        }
        case 2:{
            Mat_TALIGN[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][index] = 1;
            format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Material alignment of index %i in object ID %i has been set to CENTER", index, PLAYER_INFO[playerid][cur_obj]);
    
        }
        case 3:{
            Mat_TALIGN[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][index] = 2;
            format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Material alignment of index %i in object ID %i has been set to RIGHT", index, PLAYER_INFO[playerid][cur_obj]);
...
CMD:matsize(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing"); 
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing"); 
    if(sscanf(params, "ii", index, size)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /matsize <index> <size>");
    if(size < 1 || size > 14) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid Material size (Material size should lie in between 1 - 14)");
    if(index < 0 || index >= MAX_INDEX) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid index");
    if(!IsMatTextUsedInIndex(PLAYER_INFO[playerid][cur_svr_mid], PLAYER_INFO[playerid][cur_obj], index)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't marked any text to bold!");
    ...
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Material dimension of index %i in object ID %i has been changed to %i", index, PLAYER_INFO[playerid][cur_obj], size);
    SendClientMessage(playerid, -1, str1);
    return 1;
}
...
CMD:matbold(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing"); 
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing"); 
    if(sscanf(params, "i", index)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /matbold <index>");
    if(index < 0 || index >= MAX_INDEX) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid index");
    if(!IsMatTextUsedInIndex(PLAYER_INFO[playerid][cur_svr_mid], PLAYER_INFO[playerid][cur_obj], index)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't marked any text to bold!");
    new str1[128];
    if(Mat_TBOLD[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][index]){
        ...
        format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Material boldness of index %i in object ID %i has been toggled OFF", index, PLAYER_INFO[playerid][cur_obj]);
    }
    else{
        Mat_TBOLD[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][index] = 1;
        format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Material boldness of index %i in object ID %i has been toggled ON", index, PLAYER_INFO[playerid][cur_obj]);
    }
    new str[MAX_MATTEXT_SIZE];
    format(str, sizeof(str), "%s", GetMatTextInIndex(PLAYER_INFO[playerid][cur_svr_mid], PLAYER_INFO[playerid][cur_obj], index));
    strreplace(str, "`", " ");
...
CMD:matfont(playerid, params[])
{
    ...
    if(sscanf(params, "ii", index, id)){
        SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /matfont <index> <color>");
        return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_GREY" Type the hex of the color, (Ex - Hex of white is 0xFFFFFFFF |  0xAARRGGBB)");
    }
    if(index < 0 || index >= MAX_INDEX) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid index");
    if(!IsMatTextUsedInIndex(PLAYER_INFO[playerid][cur_svr_mid], PLAYER_INFO[playerid][cur_obj], index)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't marked any text to change color!");
    if(id < 1 || id > sizeof(Fonts)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid font face ID, Use /fontlist");
    ...
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Material font of index %i in object ID %i has been changed to \"%s\" -> ID: %i", index, PLAYER_INFO[playerid][cur_obj], Fonts[id -1], id);
    SendClientMessage(playerid, -1, str1);
    return 1;
}
...
CMD:matfontcolor(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing"); 
    if(sscanf(params, "ih", index, hex)){
        SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /matfontcolor <index> <color>");
        return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_GREY" Type the hex of the color, (Ex - Hex of white is 0xFFFFFFFF |  0xAARRGGBB)");
    }
    if(index < 0 || index >= MAX_INDEX) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid index");
    if(!IsMatTextUsedInIndex(PLAYER_INFO[playerid][cur_svr_mid], PLAYER_INFO[playerid][cur_obj], index)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't marked any text to change color!");
    ...
    SetDynamicObjectMaterialText(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_ID], index, str,
        ...
        Mat_TALIGN[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][index]);
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Material font color of index %i in object ID %i has been changed to %x", index, PLAYER_INFO[playerid][cur_obj], hex);
    SendClientMessage(playerid, -1, str1);
    return 1;
}
...
CMD:matfontsize(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing"); 
    if(PLAYER_INFO[playerid][cur_obj] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing"); 
    if(sscanf(params, "ii", index, size)) SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /matfontsize <index> <size>");
    if(index < 0 || index >= MAX_INDEX) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid index");
    if(size < 1 || size > 255) return SendClientMessage(PLAYER_INFO[playerid][cur_obj], -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid font size (font size should lie in between 1 - 255)");
    if(!IsMatTextUsedInIndex(PLAYER_INFO[playerid][cur_svr_mid], PLAYER_INFO[playerid][cur_obj], index)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't marked any text to change font size!");
    ...
    SetDynamicObjectMaterialText(OBJ_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][O_ID], index, str,
        ...
        Mat_TALIGN[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_obj]][index]);
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Material font size of index %i in object ID %i has been changed to %i", index, PLAYER_INFO[playerid][cur_obj], size);
    SendClientMessage(playerid, -1, str1);
    return 1;
}
...
CMD:tbrowse(playerid, params[])
{
    if(MENU3D_DATA[playerid][TPreviewState] == PREVIEW_STATE_ALLTEXTURES && strval(params) == 0)
    {
        ...
        SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce| "#COL_GREY"Texture selection closed!");
    }
    else
    {
        ...
        if(MENU3D_DATA[playerid][TPreviewState] == PREVIEW_STATE_NONE)
        {
            ...
            SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce| "#COL_GREY"Texture selection opened!");
            SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce| "#COL_GREY"Use numpad '6' and '4' to change the slot while 'Y' and 'N' to change within the slot");
        }
        ...
        else if(MENU3D_DATA[playerid][TPreviewState] == PREVIEW_STATE_ALLTEXTURES)
        {
            ...
            SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce| "#COL_GREY"Texture selection slot changed!");
        }
    }
    return 1;
}
...
{
    new name[30];
    if(sscanf(params, "s[30]", name)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /tsearch <name>");
    if(strlen(name) < 1 || strlen(name) > 30) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid character length");
    new model[10];
    new k = -1, p = -1, r = -1;
    new str[1024];
    new str1[1050];
...
CMD:vspawn(playerid, params[])
{
    ...
    if(sscanf(params, "s[30]", name)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce| "#COL_RED" /vspawn <name>");
    new model = -1;
    ...
    if(model == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce| "#COL_RED" No such vehicle found!");
    ...
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You spawned a \"%s\"", VehicleNames[model - 400]);
    SendClientMessage(playerid, -1, str1);
    return 1;
}
...
CMD:vadd(playerid, params[])
{
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(!IsPlayerInAnyVehicle(playerid)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You should be in a vehicle to use this command");
    if(MAP_INFO[PLAYER_INFO[playerid][cur_svr_mid]][total_vehicles] == MAX_VEH) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You have reached the maximum amount of vehicles that can be added to a map (limit: "#MAX_VEH")");
    if(GetVehicleSlotID(PLAYER_INFO[playerid][cur_svr_mid], GetPlayerVehicleID(playerid)) == -1)
    {
        ...
        new str1[128];
        format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" This vehicle has been added successfully ( Vehicle ID: %i | Map ID: %i )", id, PLAYER_INFO[playerid][cur_player_mid]);
        SendClientMessage(playerid, -1, str1);
    }
    else
    {
        ...
        GetVehicleZAngle(VEH_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][V_ID], 
            VEH_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][V_Ang]);
        new str1[128];
        format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Position for vehicle ID %i has been updated!", id);
        SendClientMessage(playerid, -1, str1);
    }
...
CMD:vcolor1(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "ii", id, color)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /vcolor1 <vehicle ID> <color ID>");
    if(id < 0 || id >= MAX_VEH) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid vehicle ID");
    if(!IsValidVehicle(VEH_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][V_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid vehicle ID");
    ...
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" The 1st color of vehicle ID %i has been changed to %i", id, color);
    SendClientMessage(playerid, -1, str1);
    return 1;
}
...
CMD:vcolor2(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "ii", id, color)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /vcolor2 <vehicle ID> <color ID>");
    if(id < 0 || id >= MAX_VEH) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid vehicle ID");
    if(!IsValidVehicle(VEH_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][V_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid vehicle ID");
    ...
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" The 2nd color of vehicle ID %i has been changed to %i", id, color);
    SendClientMessage(playerid, -1, str1);
    return 1;
}
...
CMD:vdm(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "i", id)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /vdm <vehicle ID>");
    if(id < 0 || id >= MAX_VEH) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid vehicle ID");
    if(!IsValidVehicle(VEH_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][V_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid vehicle ID");
    ...
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Vehicle has been deleted successfully ( Vehicle ID: %i | Map ID: %i )", id, PLAYER_INFO[playerid][cur_player_mid]);
    SendClientMessage(playerid, -1, str1);
    return 1;
}
...
CMD:vclone(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(MAP_INFO[PLAYER_INFO[playerid][cur_svr_mid]][total_vehicles] == MAX_VEH) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You have reached the maximum amount of vehicles that can be added to a map (limit: "#MAX_VEH")");
    if(sscanf(params, "i", id)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /vclone <vehicle ID>");
    if(id < 0 || id >= MAX_VEH) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid vehicle ID");
    if(!IsValidVehicle(VEH_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][V_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid vehicle ID");
    ...
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Vehicle ID %i has been cloned successfully!", id);
    SendClientMessage(playerid, -1, str1);
    return 1;
}
...
CMD:vmodel(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(MAP_INFO[PLAYER_INFO[playerid][cur_svr_mid]][total_vehicles] == MAX_VEH) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You have reached the maximum amount of vehicles that can be added to a map (limit: "#MAX_VEH")");
    if(sscanf(params, "ii", id, model)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /vmodel <vehicle ID> <new model ID>");
    if(id < 0 || id >= MAX_VEH) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid vehicle ID");
    if(model < 400 || model > 611) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid model ID");
    if(!IsValidVehicle(VEH_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][V_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid vehicle ID");
    DestroyVehicle(VEH_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][V_ID]);
    ...
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Model of vehicle ID %i has been changed to %i successfully!", id, model);
    SendClientMessage(playerid, -1, str1);
    return 1;
}
...
CMD:vbring(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "i", id)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /vbring <vehicle ID>");
    if(id < 0 || id >= MAX_VEH) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid vehicle ID");
    if(!IsValidVehicle(VEH_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][V_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid vehicle ID");
    new Float:x, Float:y, Float:z, Float:ang;
    GetPlayerPos(playerid, x, y, z);
    ...
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You bought vehicle ID %i to your location successfully!", id);
    SendClientMessage(playerid, -1, str1);
    return 1;
}
...
CMD:vsearch(playerid, params[])
{
    ...
    ShowPlayerDialog(playerid, DIALOG_CLOSE_DIALOG, DIALOG_STYLE_PREVIEW_MODEL, "MapForce - Vehicle browser", str, "Close", ""); 
    return 1;
}
...
{
    new id, str[128];
    if(sscanf(params, "i", id)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /anim <ID>");
    if(id < 1 || id > sizeof(AnimationData)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid anim ID");
    ApplyAnimation(playerid, AnimationData[id - 1][animlib], AnimationData[id - 1][animname], 4.0,1,0,0,0,-1);
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_GREY" ID: \"%i\" | Anim library - \"%s\" | Anim name - \"%s\"", id, AnimationData[id - 1][animlib], AnimationData[id - 1][animname]);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
{
    new name[30];
    if(sscanf(params, "s[30]", name)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /animsearch <name>");
    if(strlen(name) < 1 || strlen(name) > 30) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid character length");
    new k = -1, p = -1;
    new str[1024];
    new str1[1050];
    new substr[128];
    for(new i = 0; i < sizeof(AnimationData); i++)
    {
        k = strfind(AnimationData[i][animlib], name, true);
        p = strfind(AnimationData[i][animname], name, true);
        if(k != -1 || p != -1)
        {
            format(substr, sizeof(substr), "%i\t\t%s\t\t%s\n", i + 1, AnimationData[i][animlib], AnimationData[i][animname]);
            strcat(str, substr, sizeof(str));
        }   
    }
    ...
    format(substr, sizeof(substr), "Search results for '%s'", name);
    if(isempty(str)) return ShowPlayerDialog(playerid, DIALOG_CLOSE_DIALOG, DIALOG_STYLE_MSGBOX, substr, "Nothing found for this search!", "Close", "");
    format(str1, sizeof(str1), "Anim ID\tAnim lib\tAnim name\n%s", str); 
    ShowPlayerDialog(playerid, DIALOG_CLOSE_DIALOG, DIALOG_STYLE_MSGBOX, substr, str1, "Close", "");
    return 1;
}
...
CMD:aanim(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "i", id)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /anim <anim ID>");
    if(id < 1 || id > sizeof(AnimationData)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid anim ID");
    ...
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Animation ID %i has been applied for the actor ID %i !", id, PLAYER_INFO[playerid][cur_act]);
    SendClientMessage(playerid, -1, str1);
    return 1;
}
...
CMD:aremoveanim(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "i", id)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /aremoveanim <actor ID>");
    if(id < 0 || id > MAX_ACT) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid actor ID");
    if(!IsValidDynamicActor(ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][A_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid actor ID");
    ClearDynamicActorAnimations(ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][A_ID]);
    ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][animused] = 0;
    new str1[128];
    format(str1, sizeof(str1), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Animation has been removed from actor ID %i !", id);
    SendClientMessage(playerid, -1, str1);
    return 1;
}
...
CMD:ac(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any map for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(MAP_INFO[PLAYER_INFO[playerid][cur_player_mid]][total_actors] == MAX_ACT) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You have reached the maximum amout of actors that can be created in one map (Limit: "#MAX_ACT")");
    if(sscanf(params, "i", skin)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /ac <skin ID>");
    if(skin > 311 || skin < 0)  return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid skin ID");
    new Float:x, Float:y, Float:z, Float:xp, Float:yp;
    ...
    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Actor successfully created! ( Map ID: %s(%i) | Actor ID: %i | skin ID: %i )", MAP_INFO[PLAYER_INFO[playerid][cur_svr_mid]][M_Name], PLAYER_INFO[playerid][cur_player_mid], aid, skin);
    SendClientMessage(playerid, -1, str);
...
CMD:askinc(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(PLAYER_INFO[playerid][cur_act] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any actor for editing");
    if(!IsValidDynamicActor(ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_act]][A_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any actor for editing");
    if(sscanf(params, "i", skin)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /askinc <skin ID>");
    if(skin < 0 || skin > 312) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid skin ID");
    if(ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_act]][A_SKIN] == skin) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" This is the same skin that actor holds now!");
    ...
    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You changed skin of the Actor ID %i ( new skin ID: %i )", PLAYER_INFO[playerid][cur_act], skin);
    SendClientMessage(playerid, -1, str); 
    return 1;
}
...
CMD:ae(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any map for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing"); 
    if(PLAYER_INFO[playerid][editing] == 1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You are editing an object, finish it before editing another actor");
    if(sscanf(params, "i", id)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /ae <actor ID>");
    if(!IsValidDynamicActor(ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][A_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid actor ID");
    if(PLAYER_INFO[playerid][cur_act] == id) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You have already selected this actor for editing");
    
    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You are now editing actor ID %i ", id);
    SendClientMessage(playerid, -1, str);
...
CMD:abring(playerid, params[])
{
    ...
    SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_YELLOW" You have brought current editing actor to your location");
...
CMD:agoto(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any map for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing"); 
    if(sscanf(params, "i", id)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /agoto <actor ID>");
    if(!IsValidDynamicActor(ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][A_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid actor ID");

    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You have been teleported to actor ID %i location", id);
    SendClientMessage(playerid, -1, str);
...
CMD:aclone(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(MAP_INFO[PLAYER_INFO[playerid][cur_player_mid]][total_actors] == MAX_ACT) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You have reached the maximum amout of actors that can be created in one map (Limit: "#MAX_ACT")");
    if(sscanf(params, "i", id)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /aclone <actor ID>");
    if(!IsValidDynamicActor(ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_act]][A_ID]))  return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid actor ID");
    new aid;
    for(new i = 0; i < MAX_ACT; i++)
    {
        if(!IsValidDynamicActor(ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][i][A_ID]))
    ...
    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" Actor successfully cloned! ( Map ID: %s(%i) | Actor ID: %i | skin ID: %i )", MAP_INFO[PLAYER_INFO[playerid][cur_svr_mid]][M_Name], PLAYER_INFO[playerid][cur_player_mid], aid, ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][A_SKIN]);
    SendClientMessage(playerid, -1, str);
...
CMD:adc(playerid, params[])
{
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(PLAYER_INFO[playerid][cur_act] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    if(!IsValidDynamicActor(ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_act]][A_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any object for editing");
    
    DestroyDynamicActor(ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_act]][A_ID]);

    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You deleted the actor ID %i ( skin ID: %i )", PLAYER_INFO[playerid][cur_act], ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][PLAYER_INFO[playerid][cur_act]][A_SKIN]);
    SendClientMessage(playerid, -1, str);
...
CMD:ad(playerid, params[])
{
    ...
    if(PLAYER_INFO[playerid][mLoaded] == 0) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't loaded any maps for editing");
    if(PLAYER_INFO[playerid][cur_player_mid] == -1) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You haven't selected any map for editing");
    if(sscanf(params, "i", id)) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" /ad <actor ID>");
    if(id < 0 || id > MAX_ACT) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid Actor ID");
    if(!IsValidDynamicActor(ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][A_ID])) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Invalid actor ID");
    if(id == PLAYER_INFO[playerid][cur_act]) return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You are currently editing this actor ( use /adc )");

    DestroyDynamicActor(ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][A_ID]);

    new str[128];
    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_YELLOW" You deleted the actor ID %i ( actor ID: %i )", id, ACT_INFO[PLAYER_INFO[playerid][cur_svr_mid]][id][A_SKIN]);
    SendClientMessage(playerid, -1, str);
...
CMD:asearch(playerid, params[])
{
    ...
    ShowPlayerDialog(playerid, DIALOG_CLOSE_DIALOG, DIALOG_STYLE_PREVIEW_MODEL, "MapForce - Skin browser", str, "Close", ""); 
    return 1;
}
    ...
    switch(dialogid) 
    {
        ...
        case DIALOG_CMDS:
        {
            if(response)
            {
                ...
                strcat(str, ""#COL_WHITE"/animsearch\t\t\t\t\t\t- Search animation\n", sizeof(str));
                ...
                ShowPlayerDialog(playerid, DIALOG_CMDS2, DIALOG_STYLE_MSGBOX, "MapForce - Commands - Page(2/3)", str, "Next", "Back");
            }
            else return 1;
            return 1;
        }
        case DIALOG_CMDS2:
        {
            if(response)
            {
                ...
                strcat(str, ""#COL_WHITE"/vspawn\t\t\t\t\t\t- Spawn a vehicle\n", sizeof(str));
                ...
                ShowPlayerDialog(playerid, DIALOG_CMDS3, DIALOG_STYLE_MSGBOX, "MapForce - Commands - Page(3/3)", str, "Back", "Close");
            }
            else return PC_EmulateCommand(playerid, "/editorcmds");
            return 1;
        }
        case DIALOG_CMDS3:
        {
        ...
        case DIALOG_MAPS:
        {
            if(response)
            {
                ...
                new str[128];
                format(str, sizeof(str), "MapForce - Management Tools for Map '%s'", mDialog[playerid][listitem]);
            ...
            if(response)
            {
                ...
                switch(listitem)
                {
                    case 0:
                    {
                        ...
                        ShowPlayerDialog(playerid, DIALOG_MAP_INFO, DIALOG_STYLE_MSGBOX, "MapForce - Map Information", str, "Back", "Close");
                    }
                    case 1:
                    {
                        new str[256];
                        ...
                        ShowPlayerDialog(playerid, DIALOG_MAP_NAME, DIALOG_STYLE_INPUT, "MapForce - Map Name", str, "Enter", "Back");
                    }
                    case 2:
                    {
                        new str[256];
                        ...
                        ShowPlayerDialog(playerid, DIALOG_MAP_PASS, DIALOG_STYLE_INPUT, "MapForce - Map Password", str, "Enter", "Back");
                    }
                    case 3:
                    {
                        new str[256];
                        format(str, sizeof(str), "Are you sure that you want to\ndelete Map '%s' created by '%s'", cur_map[playerid], dini_Get(str_, "Creator"));
                        ShowPlayerDialog(playerid, DIALOG_MAP_DELETE, DIALOG_STYLE_MSGBOX, "MapForce - Map Password", str, "Delete", "Back");
                    }
                }
            }
            else return 1;
        ...
        case DIALOG_MAP_INFO:
        {
            ...
            {
                new str[128];
                format(str, sizeof(str), "MapForce - Management Tools for Map '%s'", cur_map[playerid]);
                ShowPlayerDialog(playerid, DIALOG_MANAGE_MAP, DIALOG_STYLE_LIST, str, "Map Information\nEdit Name\nEdit Password\nDelete Map", "Select", "Close");
            }
            else return 1;
            return 1;
        ...
        case DIALOG_MAP_NAME:
        {
            if(response)
            {
                ...
                if(strlen(inputtext) > 20 || strlen(inputtext) < 4 || !IsValidMapName(inputtext))  
                {
                    ...
                    format(str, sizeof(str), "Enter a desired name as the new name\nfor map '%s'\nIf someone has loaded the map inform\nation about name change will be sent\nto him [ Creator - %s ]", cur_map[playerid], dini_Get(str_, "Creator"));
                    ShowPlayerDialog(playerid, DIALOG_MAP_NAME, DIALOG_STYLE_INPUT, "MapForce - Map Name", str, "Enter", "Back");
                    return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Map name should contain only 4 - 20 characters with lowercase letters or numbers");
                }
                ...
                if(dini_Exists(path))
                {
                    ...
                    format(str, sizeof(str), "Enter a desired name as the new name\nfor map '%s'\nIf someone has loaded the map inform\nation about name change will be sent\nto him [ Creator - %s ]", cur_map[playerid], dini_Get(str_, "Creator"));
                    ShowPlayerDialog(playerid, DIALOG_MAP_NAME, DIALOG_STYLE_INPUT, "MapForce - Map Name", str, "Enter", "Back");
                    return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" A map with this name already exsists");
                }
                ...
                if(id != -1)
                {
                    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_GREY" Admin '%s[%i]' has changed the name of your map '%s' to '%s'", PlayerName(playerid), playerid, cur_map[playerid], inputtext);
                ...
                format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_GREY" You changed the name of map '%s' to '%s'", cur_map[playerid], inputtext);
                SendClientMessage(playerid, -1, str);
        ...
        case DIALOG_MAP_PASS:
        {
            if(response)
            {
                ...
                if(strlen(inputtext) < 4 || strlen(inputtext) > 20)
                {
                    ...
                    format(str, sizeof(str), "Enter a desired password as the new password\nfor map '%s'\nIf someone has loaded the map information\nabout password change will be sent to him\n[ Creator - %s ]", cur_map[playerid], dini_Get(str_, "Password"));
                    ShowPlayerDialog(playerid, DIALOG_MAP_PASS, DIALOG_STYLE_INPUT, "MapForce - Map Password", str, "Enter", "Back");
                    return SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" Map password should contain only 4 - 20 characters with lowercase letters or numbers"); 
                }
                ...
                if(id != -1)
                {
                    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_GREY" Admin '%s[%i]' has changed the password of your map '%s' to '%s'", PlayerName(playerid), playerid, cur_map[playerid], inputtext);
                    SendClientMessage(id, -1, str);
                }

                format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_GREY" You changed the password of map '%s' to '%s'", cur_map[playerid], inputtext);
                SendClientMessage(playerid, -1, str);
        ...
        case DIALOG_MAP_DELETE:
        {
            ...
            if(response)
            {
                ...
                if(id != -1)
                {
                    ...
                    format(str, sizeof(str), ""#COL_BLUE"|MapForce|"#COL_GREY" Admin '%s[%i]' has force unloaded and deleted one of your loaded maps ( deleted map name: %s )", PlayerName(playerid), playerid, cur_map[playerid]);
                    SendClientMessage(id, -1, str);
                }
        ...
        {   
            if(response)
            {
                ...
                {
                    SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" This map is loaded only for view, You can't select it for editing!");
                    return PC_EmulateCommand(playerid, "/em");
                }

                if(PLAYER_INFO[playerid][cur_player_mid] == mid)
                {
                    SendClientMessage(playerid, -1, ""#COL_BLUE"|MapForce|"#COL_RED" You have already selected this map for editing!");
                    return PC_EmulateCommand(playerid, "/em");
                }
            ...
            if(response)
            {
                if(strlen(inputtext) > 0)
                {
                    ...
                    if(iMatches == 0)
                    {
                        ...
                        return ShowPlayerDialog(playerid, DIALOG_MODEL_SEARCH, DIALOG_STYLE_INPUT, "MapForce - Search object models", szDialog, "Search", "Close");
                    }
                    
                    new str[128];
                    format(str, sizeof(str), "MapForce - Search results for '%s'", inputtext);
                    ShowPlayerDialog(playerid, 567, DIALOG_STYLE_PREVIEW_MODEL, str, szDialog, "Search", "Close"); 
                }
                else ShowPlayerDialog(playerid, DIALOG_MODEL_SEARCH, DIALOG_STYLE_INPUT, "MapForce - Search Object Models", "No matches were found", "Search", "Close");
            }
            else return 1;
            return 1;
        }
...
{
    Streamer_SetVisibleItems(STREAMER_TYE_OBJECT, 500);
    return 1;
}