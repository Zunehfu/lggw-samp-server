...
Version - v 1.00
Forum - https://www.lazergaming.net
...
public OnGameModeInit()
{
	...
	if(rows < sizeof(zoneinfo))
	{
		for(new i = 0; i < sizeof(zoneinfo); i ++)
		{
			if(i > (rows - 1))
			{
				...
				mysql_format(Database, str, sizeof(str), "INSERT INTO `Zones` (`Zone_owned_team_ID`) VALUES('%d')", TeamRandoms[Rand] + 1);
				mysql_query(Database, str, false);
			} 
		}
	...
	CreateDynamicMapIcon(2128.5830,-1140.2947,25.2329, 55, 0, -1, -1, -1, 300.0, MAPICON_GLOBAL);   
	CreateDynamicMapIcon(1518.2750,-1465.3423,9.5000, 27, 0, -1, -1, -1, 300.0, MAPICON_GLOBAL);
...
public OnPlayerDataCheck(playerid, corrupt_check)
{
    ...
    {
        new str[128];
        format(str, sizeof(str), "* \"%s[%i]\" has been kicked due to MySQL system overload!", userinfo[playerid][pname], playerid);
...
Dialog:DIALOG_GANG_CONFIRMATION(playerid, response, listitem, inputtext[])
{
	if(response)
	{
        ...
        mysql_format(Database, str, sizeof(str), "UPDATE `Gangs` SET `Name` = '%e', `Tag` = '%e', `Color` = -1, `HQ` = 0, `HQ_ID` = -1 WHERE `Gang_ID`  = %d LIMIT 1", tempgname[playerid], tempgtag[playerid], key + 1);
		mysql_query(Database, str, false);
		...
		mysql_format(Database, str, sizeof(str), "UPDATE `Users` SET `In_gang` = 1, `Gang_ID` = %d, `Gang_level` = 4, `Gang_skin` = 1 WHERE `User_ID` = %d LIMIT 1", userinfo[playerid][gid] + 1, userinfo[playerid][pid]);
		mysql_tquery(Database, str);
	}
	else Dialog_Close(playerid);
	return 1;
	...
	if(response)
	{
		...
		else if(isequal(inputtext, "2"))
		{
			...
			else
			{
				...
				mysql_format(Database, str, sizeof(str), "UPDATE `Gangs` SET `HQ` = 1, `HQ_ID` = %d WHERE `Gang_ID` = %d LIMIT 1", i + 1, userinfo[playerid][gid] + 1);
				mysql_tquery(Database, str);

				mysql_format(Database, str, sizeof(str), "UPDATE `Houses` SET `House_owned` = 1, `House_owned_team_ID` = %d WHERE `House_ID` = %d LIMIT 1", userinfo[playerid][gid] + 1, i + 1);
				mysql_tquery(Database, str);
			}
		}
		else
...
Dialog:DIALOG_GANGS(playerid, response, listitem, inputtext[])
{
	if(response)
	{
		if(listitem > 6)
		{
			...
			mysql_format(Database, str, sizeof(str), "SELECT `Name`, `Gang_level` FROM `Users` WHERE `Gang_ID` = %d AND `In_gang` = 1 ORDER BY `Gang_level` DESC LIMIT "#MAX_GANG_MEMBERS"", val + 1);
			new Cache:r = mysql_query(Database, str);

            printf("Gang_ID - %i", val + 1);
...
{   
    new str[80];
    mysql_format(Database, str, sizeof(str), "SELECT `User_ID` FROM `Users` WHERE `Gang_ID` = %d AND `In_gang` = 1", g_id + 1);
    new Cache:r = mysql_query(Database, str);
    new rows; 
    cache_get_row_count(rows);
    cache_delete(r); 
...
CMD:gang(playerid, params[])
{
	...
	if(isequal(params, "", true)) return SendClientMessage(playerid, -1, "{ff0000}Invalid gang command, Use '/gang help' for more details.");
	...
	else if(!sscanf(params, "'accept'D(-1)", tmp))
	{
		...
		mysql_format(Database, str, sizeof(str), "UPDATE `Users` SET `In_gang` = 1, `Gang_ID` = %d, `Gang_level` = 1, `Gang_skin` = 1 WHERE `User_ID` = %d LIMIT 1", idog + 1, userinfo[tmp][pid]);
		mysql_tquery(Database, str);
        ...
        mysql_format(Database, str, sizeof(str), "UPDATE `Users` SET `In_gang` = 1, `Gang_ID` = %d, `Gang_skin` = 1, `Gang_level` = 1 WHERE `User_ID` = %d LIMIT 1", idog + 1, userinfo[tmp][pid]);
        mysql_tquery(Database, str);
	}
	else if(isequal(params, "destroy", true))
	{
        ...
        mysql_format(Database, str, sizeof(str), "UPDATE `Users` SET `In_gang` = 0 WHERE (`Gang_ID` = %d AND `In_gang` = 1) LIMIT "#MAX_GANG_MEMBERS"", userinfo[playerid][gid] + 1);
        mysql_tquery(Database, str);
	...
	{
		if(userinfo[playerid][ingang] == 0) return SendClientMessage(playerid, -1, "{ff0000}You are not in a gang.");
		mysql_format(Database, str, sizeof(str), "SELECT `Gang_level`, `Name`, `Last_Online` FROM `Users` WHERE `Gang_ID` = %d AND `In_gang` = 1 ORDER BY `Gang_level` DESC LIMIT "#MAX_GANG_MEMBERS"", userinfo[playerid][gid] + 1);
		new Cache:r = mysql_query(Database, str);
	...
	else if(!sscanf(params, "'remove'S('-1')["#MAX_PLYER_NAME"]", name))
	{
		...
		mysql_format(Database, gstr, sizeof(gstr), "SELECT `User_ID`, `Gang_level` FROM `Users` WHERE `Name` = '%e' AND `In_gang` = 1 AND `Gang_ID` = %d LIMIT 1", name, userinfo[playerid][gid] + 1);
...
CMD:dm2(playerid, params[])
{
	...
	TextDrawShowForPlayer(playerid, DM__[1]);
	PlayerTextDrawShow(playerid, DM_1[playerid]);
	PlayerTextDrawSetString(playerid, DM_1[playerid], "~g~kills:_0______________ ~r~deaths:_0______________ ~y~killing|====|spree:_0______________ ~p~ratio:_0.00______________");
